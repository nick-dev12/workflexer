...
if (!isset($_GET['pageCategorie']) || empty($_GET['pageCategorie'])) {
    header('Location: ../entreprise/offre_suprimer.php');
    exit;
}
...
$categorie = $_GET['pageCategorie'];

// Récupérer les offres supprimées de cette catégorie
$offresCategorie = getOffresEmplois_suprimer_categorie($db, $_SESSION['compte_entreprise'], $categorie);
...
<head>
    ...
    <link rel="stylesheet" href="../css/offres_supprimer_categorie.css">
                        ...
                        <div class="offer-card">
                            ...
                            <div class="offer-content">
                                ...
                                <div class="offer-actions">
                                    <a href="?pageCategorie=<?= urlencode($categorie) ?>&restorer=<?= $offre['offre_id'] ?>"
                                        class="action-btn restore-btn"
                                        onclick="return confirm('Êtes-vous sûr de vouloir restaurer cette offre ?');">
                                        <i class="fas fa-undo-alt"></i>
                                        <span>Restaurer</span>
                                    </a>
                                    <a href="?pageCategorie=<?= urlencode($categorie) ?>&offre_id_suprime=<?= $offre['offre_id'] ?>"
                                        class="action-btn delete-btn"
                                        onclick="return confirm('Êtes-vous sûr de vouloir supprimer définitivement cette offre ? Cette action est irréversible.');">
                                        <i class="fas fa-trash-alt"></i>
                                        <span>Supprimer</span>
                                    </a>
                                </div>
