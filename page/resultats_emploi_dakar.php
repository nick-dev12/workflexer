...
// Vérifier si des critères de recherche existent pour EmploiDakar
if (!isset($_SESSION['criteres_emploi_dakar'])) {
    header('Location: offres_emploi_dakar.php');
    exit();
}
...
$recherche = $_SESSION['criteres_emploi_dakar']['search'];
...
if (!empty($recherche)) {
    ...
    $bindParams[] = $searchParam;
    $params[':recherche'] = "%$recherche%"; // Gardé pour l'affichage du débogage
}
    ...
    if (!empty($recherche)) {
        ...
        try {
            $testQuery = "SELECT offre_id, titre, entreprise, SUBSTRING(description_poste, 1, 100) as desc_sample 
                         FROM scrap_emploi_emploidakar 
                         WHERE titre LIKE ? OR description_poste LIKE ? 
                         LIMIT 5";
            $testStmt = $db->prepare($testQuery);
            $searchTerm = "%{$recherche}%";
            $testStmt->bindParam(1, $searchTerm);
            $testStmt->bindParam(2, $searchTerm);
            $testStmt->execute();
            ...
            if (!$foundResults) {
                ...
                foreach ($testTerms as $term) {
                    $genericQuery = "SELECT COUNT(*) FROM scrap_emploi_emploidakar WHERE titre LIKE ? OR description_poste LIKE ?";
                    $genericStmt = $db->prepare($genericQuery);
                    $genericTerm = "%{$term}%";
                    $genericStmt->bindParam(1, $genericTerm);
                    $genericStmt->bindParam(2, $genericTerm);
                    $genericStmt->execute();
...
<head>
    ...
    <meta name="description" content="Résultats de recherche pour les offres d'EmploiDakar sur Work-Flexer.">
    <title>Résultats de recherche - EmploiDakar | WorkFlexer</title>
...
<body>
    ...
    <div class="results-header">
        <div class="container">
            <h1>Résultats de recherche - EmploiDakar</h1>
            ...
            <div class="actions">
                <a href="offres_emploi_dakar.php?reset=1" class="btn-new-search">Nouvelle recherche</a>
                <a href="offres_emploi_dakar.php" class="btn-all-jobs">Voir toutes les offres</a>
            </div>
        ...
        <div class="other-sources">
            ...
            <div class="sources-links">
                ...
                <a href="offres_senjob.php" class="source-link">
                    <i class="fas fa-search"></i> Rechercher sur SenJob
                </a>
            </div>
        </div>
    </section>
