/**
 * Script pour la page du secteur Tourisme et Hôtellerie
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Ajout d'effets visuels spécifiques au secteur tourisme
    enhanceVisualElements();

    // Initialisation du convertisseur de devises FCFA/Euro
    initCurrencyConverter();

    // Initialisation des statistiques du marché touristique africain
    initAfricanTourismStats();
});
 ...
 */
function initAfricanTourismStats() {
    // Création d'un élément pour afficher les statistiques du marché
    const statsContainer = document.createElement('div');
    statsContainer.className = 'african-tourism-stats';
    statsContainer.innerHTML = `
        <div class="stats-header">
            <h3>Statistiques du tourisme en Afrique</h3>
            <p>Données actualisées 2023-2024</p>
        </div>
        <div class="stats-content">
            <div class="stats-item">
                <span class="stats-number">+8,5%</span>
                <span class="stats-label">Contribution au PIB africain</span>
            </div>
            <div class="stats-item">
                <span class="stats-number">+67M</span>
                <span class="stats-label">Visiteurs internationaux annuels</span>
            </div>
            <div class="stats-item">
                <span class="stats-number">+35%</span>
                <span class="stats-label">Croissance du tourisme domestique</span>
            </div>
        </div>
    `;
...
/**
 * Ajoute des éléments visuels spécifiques au contexte touristique africain
 */
function addAfricanTourismElements() {
    // Cette fonction pourrait être utilisée pour ajouter dynamiquement
    // des éléments spécifiques au contexte africain

    // Par exemple, ajouter des badges pour les formations en écotourisme
    const tourismItems = document.querySelectorAll('.education-path li');
    tourismItems.forEach(item => {
        if (item.textContent.includes('écotourisme') || item.textContent.includes('communautaire')) {
            const badge = document.createElement('span');
            badge.className = 'african-tourism-tag';
...
/**
 * Statistiques touristiques pour l'Afrique
 * Basées sur les données 2023-2024
 */
const africanTourismStats = {
    // Répartition des arrivées touristiques par région (2023)
    touristArrivals: {
        'Afrique du Nord': 37.4, // en pourcentage
        'Afrique de l\'Ouest': 19.6,
        'Afrique de l\'Est': 25.8,
        'Afrique Australe': 14.2,
        'Afrique Centrale': 3.0
    },

    // Principales destinations touristiques en Afrique (2023)
    topDestinations: [
        'Maroc',
        'Égypte',
        'Afrique du Sud',
 ...
 */
console.log('%cBienvenue sur la page Tourisme et Hôtellerie - Secteur Afrique', 'font-size: 14px; color: #1abc9c; font-weight: bold');
console.log('%cDécouvrez les opportunités touristiques en Afrique francophone', 'font-style: italic; color: #333'); 