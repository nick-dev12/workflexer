/**
 * Script pour la page du secteur Transport et Logistique
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Ajout d'effets visuels spécifiques au secteur transport
    enhanceVisualElements();

    // Initialisation du convertisseur de devises FCFA/Euro
    initCurrencyConverter();

    // Initialisation des statistiques du marché transport africain
    initAfricanTransportStats();
});
...
function enhanceVisualElements() {
    ...
    educationItems.forEach(item => {
        if (item.textContent.includes('CESAG') ||
            item.textContent.includes('ISCAE') ||
            item.textContent.includes('UCAD') ||
            item.textContent.includes('2IE')) {
            const badge = document.createElement('span');
            badge.className = 'african-transport-tag';
 ...
 */
function initAfricanTransportStats() {
    // Création d'un élément pour afficher les statistiques du marché
    const statsContainer = document.createElement('div');
    statsContainer.className = 'african-transport-stats';
    statsContainer.innerHTML = `
        <div class="stats-header">
            <h3>Statistiques du transport en Afrique</h3>
            <p>Données actualisées 2023-2024</p>
        </div>
        <div class="stats-content">
            <div class="stats-item">
                <span class="stats-number">7,5%</span>
                <span class="stats-label">Contribution au PIB africain</span>
            </div>
            <div class="stats-item">
                <span class="stats-number">+22%</span>
                <span class="stats-label">Croissance prévue d'ici 2025</span>
            </div>
            <div class="stats-item">
                <span class="stats-number">+45%</span>
                <span class="stats-label">Augmentation des échanges intra-africains</span>
            </div>
        </div>
    `;
...
/**
 * Ajoute des éléments visuels spécifiques au contexte transport africain
 */
function addAfricanTransportElements() {
    // Cette fonction pourrait être utilisée pour ajouter dynamiquement
    // des éléments spécifiques au contexte africain

    // Par exemple, ajouter des badges pour les formations en logistique régionale
    const transportItems = document.querySelectorAll('.education-path li');
    transportItems.forEach(item => {
        if (item.textContent.includes('régional') || item.textContent.includes('africain')) {
            const badge = document.createElement('span');
            badge.className = 'african-transport-tag';
            badge.textContent = 'Spécialité Africaine';
            item.appendChild(badge);
        }
    });
}
...
/**
 * Statistiques transport pour l'Afrique
 * Basées sur les données 2023-2024
 */
const africanTransportStats = {
    // Répartition des modes de transport en Afrique (2023)
    transportModes: {
        'Transport routier': 76.5, // en pourcentage
        'Transport ferroviaire': 7.2,
        'Transport maritime': 12.8,
        'Transport aérien': 3.5
    },

    // Principaux corridors de transport en Afrique (2023)
    topCorridors: [
        'Corridor Nord (Mombasa-Kampala-Kigali)',
        'Corridor Central (Dar es Salaam-Kigali)',
        'Corridor Abidjan-Lagos',
        'Corridor Dakar-Bamako',
        'Corridor Nord-Sud (Durban-Lubumbashi)',
        'Corridor Trans-Maghreb',
        'Corridor Djibouti-Addis-Abeba',
        'Corridor Walvis Bay-Ndola-Lubumbashi'
    ],

    // Tendances transport en Afrique (2023-2024)
    trends: [
        'Logistique Verte',
        'Digitalisation et Solutions Mobiles',
        'Intégration Régionale',
        'Logistique Urbaine Africaine'
    ]
};
 ...
 */
console.log('%cBienvenue sur la page Transport et Logistique - Secteur Afrique', 'font-size: 14px; color: #2980b9; font-weight: bold');
console.log('%cDécouvrez les opportunités logistiques en Afrique francophone', 'font-style: italic; color: #333'); 